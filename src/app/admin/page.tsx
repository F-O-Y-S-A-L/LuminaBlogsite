...
export default function Admin() {
  ...
  const [stats, setStats] = useState({ totalPosts: 0, totalUsers: 0, systemIntegrity: '---' });
  ...
  useEffect(() => {
    if (!user && !loadingStats /* using loadingStats as a proxy for initial load */) {
      router.push('/login');
    }
  ...
  if (!user || user.role !== 'admin') return <div className="max-w-7xl mx-auto py-20 text-center">Unauthorised access detected. Admin credentials required.</div>;
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({
          title: '', content: '', excerpt: '', category: 'Tech',
          coverImage: ''
        });
        setView('list');
      } else {
  ...
  const handleDeletePost = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setRecentActivity(recentActivity.filter(activity => activity.id !== id));
        ...
        <div className="space-y-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-xl shadow-slate-200/50 space-y-4">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Transmissions</span>
              <p className="text-4xl font-black text-slate-900 font-outfit">
                {loadingStats ? '---' : `${stats.totalPosts} blog`}
              </p>
            </div>
            <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-xl shadow-slate-200/50 space-y-4">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Identity Nodes</span>
              <p className="text-4xl font-black text-slate-900 font-outfit">
                {loadingStats ? '---' : `${stats.totalUsers} ID`}
              </p>
            </div>
            <div className="p-10 bg-white border border-slate-100 rounded-[2.5rem] shadow-xl shadow-slate-200/50 space-y-4">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">System Integrity</span>
          ...
          <div className="bg-white border border-slate-100 rounded-[3rem] shadow-2xl shadow-indigo-100/50 overflow-hidden">
            <header className="p-10 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
              <h2 className="text-xl font-black text-slate-900 font-outfit uppercase tracking-tight">Recent Activity Log</h2>
              <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-600">LIVE // REAL-TIME</span>
            </header>
            <div className="divide-y divide-slate-50">
              {!loadingStats && recentActivity.length > 0 ? (
                recentActivity.map((activity, i) => (
                  <div key={activity.id} className="p-10 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center gap-8">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-400 font-medium mt-1 uppercase tracking-widest">
                          {activity.type === 'TRANS_SYNC' ? 'Applied // Sync Complete' : 'Manual Intervention'}
                        </div>
                      </div>
                    </div>
                ...
                <div className="p-20 text-center text-slate-300 text-[10px] font-bold uppercase tracking-widest">
                  {loadingStats ? 'Scanning Registry...' : 'No activity nodes detected in current sequence.'}
                </div>
              )}
            </div>
          </div>
        ...
        >
          ...
          <form onSubmit={handleCreatePost} className="space-y-16">
            <div className="space-y-12">
              <div className="space-y-4">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4">Entry Label (Title)</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                <div className="space-y-4">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4">Classification</label>
                  <select 
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4">Visual Core (Image URL)</label>
                <input
                  type="text"
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4">Fragment Preview (Excerpt)</label>
                <textarea
                  required
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-4">Master Manuscript (Content)</label>
                <textarea
                  required
